...
export const getOrderTotalWeightGrams = (order) => {
  return (order.items || []).reduce((acc, item) => {
    ...
    const unitWeight = toGrams(product.weight);
...
export const quoteManualShippingForOrder = (order) => {
  ...
  if (!regionKey) {
    throw new Error("La región ingresada no tiene tarifa configurada");
  }
  ...
  if (!tier) {
    throw new Error("El peso total supera el máximo permitido para tarifa manual");
  }
  ...
  if (!amount) {
    throw new Error("No existe tarifa para esa región y rango de peso");
  }
